...
export default function Module8Page() {
  const { lang } = useApp()
  const titleRu = 'Модуль 8 — Production Deployment'
  const titleEn = 'Module 8 — Production Deployment'
  const descRu = 'Агент на VPS сервере 24/7. Docker, Hostinger, мониторинг, backup стратегия.'
  const descEn = 'Agent on VPS server 24/7. Docker, Hostinger, monitoring, backup strategy.'
  const durRu = '⏱ 90 мин · 🚀 Продвинутый'
  const durEn = '⏱ 90 min · 🚀 Advanced'
  const navPrevRu = '← Модуль 7: Автоматизация'
  const navPrevEn = '← Module 7: Automation'
  const navNextRu = 'Модуль 9: Продвинутые кейсы →'
  const navNextEn = 'Module 9: Advanced Cases →'
  ...
  return (
    <div style={{ maxWidth: '860px', margin: '0 auto', padding: '32px 20px 80px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '32px' }}>
        ...
        <Link href="/modules/9" style={{ color: '#00d4ff', textDecoration: 'none', fontSize: '14px', fontWeight: '600' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
        ...
        <Link href="/modules/9" style={{ color: '#00d4ff', textDecoration: 'none', fontSize: '14px', fontWeight: '600', padding: '10px 0' }}>
          {lang === 'ru' ? navNextRu : navNextEn}
        </Link>
      </div>
    </div>
