...
export default function Module6Page() {
  const { lang } = useApp()
  const titleRu = 'Модуль 6 — Субагенты и координация'
  const titleEn = 'Module 6 — Sub-agents & Coordination'
  const descRu = 'Multi-agent архитектура. Параллельные агенты для сложных задач. Parent↔Child паттерн.'
  const descEn = 'Multi-agent architecture. Parallel agents for complex tasks. Parent-Child pattern.'
  const durRu = '⏱ 75 мин · 🤖 Практика'
  const durEn = '⏱ 75 min · 🤖 Practice'
  const navPrevRu = '← Модуль 5: Безопасность'
  const navPrevEn = '← Module 5: Security'
  const navNextRu = 'Модуль 7: Автоматизация →'
  const navNextEn = 'Module 7: Automation →'

  return (
    <div style={{ maxWidth: '860px', margin: '0 auto', padding: '32px 20px 80px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '32px' }}>
        <Link href="/modules/5" style={{ color: '#00d4ff', textDecoration: 'none', fontSize: '14px', fontWeight: '600' }}>
          {lang === 'ru' ? navPrevRu : navPrevEn}
        </Link>
        <Link href="/modules/7" style={{ color: '#00d4ff', textDecoration: 'none', fontSize: '14px', fontWeight: '600' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
        <Link href="/modules/5" style={{ color: '#00d4ff', textDecoration: 'none', fontSize: '14px', fontWeight: '600', padding: '10px 0' }}>
          {lang === 'ru' ? navPrevRu : navPrevEn}
        </Link>
        <Link href="/modules/7" style={{ color: '#00d4ff', textDecoration: 'none', fontSize: '14px', fontWeight: '600', padding: '10px 0' }}>
          {lang === 'ru' ? navNextRu : navNextEn}
        </Link>
      </div>
    </div>
