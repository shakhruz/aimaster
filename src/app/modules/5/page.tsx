...
export default function Module5Page() {
  const { lang } = useApp()
  const titleRu = 'Модуль 5 — Безопасность агента'
  const titleEn = 'Module 5 — Agent Security'
  const descRu = 'Защита от атак, prompt injection defense, ACCESS.md контроль. Security audit.'
  const descEn = 'Attack protection, prompt injection defense, ACCESS.md control. Security audit.'
  const durRu = '⏱ 60 мин · 🔒 Теория + Практика'
  const durEn = '⏱ 60 min · 🔒 Theory + Practice'
  const navPrevRu = '← Модуль 4: Скиллы'
  const navPrevEn = '← Module 4: Skills'
  const navNextRu = 'Модуль 6: Субагенты →'
  const navNextEn = 'Module 6: Sub-agents →'
  ...
  return (
    <div style={{ maxWidth: '860px', margin: '0 auto', padding: '32px 20px 80px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
        <Link href="/modules/4" style={{ color: '#00d4ff', textDecoration: 'none', fontSize: '14px', fontWeight: '600', padding: '10px 0' }}>
          {lang === 'ru' ? navPrevRu : navPrevEn}
        </Link>
