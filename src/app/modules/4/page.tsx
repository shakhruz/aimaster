...
export default function Module4Page() {
  const { lang } = useApp()
  const titleRu = 'Модуль 4 — Скиллы и плагины'
  const titleEn = 'Module 4 — Skills & Plugins'
  const descRu = 'Установка проверенных скиллов из ClawHub. 5700+ расширений, но 12-20% вредоносны.'
  const descEn = 'Installing verified skills from ClawHub. 5700+ extensions, but 12-20% are malicious.'
  const durRu = '⏱ 60 мин · 🧩 Практика'
  const durEn = '⏱ 60 min · 🧩 Practice'
  const navPrevRu = '← Модуль 3: Каналы'
  const navPrevEn = '← Module 3: Channels'
  const navNextRu = 'Модуль 5: Безопасность →'
  const navNextEn = 'Module 5: Security →'
  ...
  return (
    <div style={{ maxWidth: '860px', margin: '0 auto', padding: '32px 20px 80px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
        <Link href="/modules/3" style={{ color: '#00d4ff', textDecoration: 'none', fontSize: '14px', fontWeight: '600', padding: '10px 0' }}>
          {lang === 'ru' ? navPrevRu : navPrevEn}
        </Link>
