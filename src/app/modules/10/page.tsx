...
export default function Module10Page() {
  const { lang } = useApp()
  const titleRu = 'Модуль 10 — Финальный проект'
  const titleEn = 'Module 10 — Final Project'
  const descRu = 'Собери всё вместе: агент с SOUL.md, 3+ скиллами, Telegram, памятью и автоматизацией.'
  const descEn = 'Put it all together: agent with SOUL.md, 3+ skills, Telegram, memory and automation.'
  const durRu = '⏱ 60 мин · 🏆 Финал'
  const durEn = '⏱ 60 min · 🏆 Final'
  const navPrevRu = '← Модуль 9: Продвинутые кейсы'
  const navPrevEn = '← Module 9: Advanced Cases'
  const navNextRu = 'Завершить курс →'
  const navNextEn = 'Complete Course →'
  ...
  return (
    <div style={{ maxWidth: '860px', margin: '0 auto', padding: '32px 20px 80px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
        ...
        <Link href="/" style={{ color: '#00d4ff', textDecoration: 'none', fontSize: '14px', fontWeight: '600', padding: '10px 0' }}>
          {lang === 'ru' ? navNextRu : navNextEn}
        </Link>
      </div>
    </div>
